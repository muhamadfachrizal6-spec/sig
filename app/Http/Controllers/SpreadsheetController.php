    ...
    public function index()
    {
        try {
            ...
            foreach ($dataRows as $row) {
                ...
                $yield = isset($row[40]) ? (is_numeric(cleanValue($row[40])) ? cleanValue($row[40]) / 100 : 0) : 0;
                ...
                $DAR = isset($row[41]) ? (is_numeric(cleanValue($row[41])) ? cleanValue($row[41]) : 0) : 0;
                $DER = isset($row[42]) ? (is_numeric(cleanValue($row[42])) ? cleanValue($row[42]) : 0) : 0;

                $ROE = isset($row[34]) ? (is_numeric(cleanValue($row[34])) ? cleanValue($row[34]) : 0) : 0;
                $GPM = isset($row[36]) ? (is_numeric(cleanValue($row[36])) ? cleanValue($row[36]) : 0) : 0;
                $NPM = isset($row[38]) ? (is_numeric(cleanValue($row[38])) ? cleanValue($row[38]) : 0) : 0;

                $EPS = isset($row[24]) ? (is_numeric(cleanValue($row[24])) ? cleanValue($row[24]) : 0) : 0;
                $PER = isset($row[25]) ? (is_numeric(cleanValue($row[25])) ? cleanValue($row[25]) : 0) : 0;
                $BVPS = isset($row[26]) ? (is_numeric(cleanValue($row[26])) ? cleanValue($row[26]) : 0) : 0;
                $PBV = isset($row[27]) ? (is_numeric(cleanValue($row[27])) ? cleanValue($row[27]) : 0) : 0;
            ...
            foreach ($kodeSahamList as $kodeSaham => $entries) {
                ...
                foreach ($entries as $entry) {
                    ...
                    $netProfitCurrent = $entry['net_profit']; // Ambil net profit untuk tahun ini dan kuartal ini
                    ...
                    if (isset($netProfitData[$kodeSaham][$year - 1][$quarter])) {
                        ...
                        $netProfitPrevious = 0; // Tidak ada data untuk tahun sebelumnya
                    }
